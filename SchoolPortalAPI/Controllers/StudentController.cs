...
//        }
            ...
            [FromForm] IFormFile? file)
        {
            ...
            var submission = new ExerciseStuTeach
            {
                ...
                Description = description,
                Date = DateTime.Now.ToShamsi(),
                Filename = originalFileName,  // ✓ SAVE ORIGINAL FILENAME (or null if no file)
                Answerimage = fileName,        // ✓ SAVE GENERATED FILENAME (or null if no file)
                Score = null // Pending grading
            };
        ...
        // Update assignment answer (replace existing submission)
        // ──────────────────────────────────────────────────────────────
        [HttpPost("update/assignment/{userId}/{assignmentId}/{isFile}")]
        public async Task<IActionResult> UpdateAssignmentAnswer(
            long userId,
            long assignmentId,
            bool isFile,
            ...
            [FromForm] IFormFile? file)
        {
            ...
            // Handle file upload
            string? fileName = isFile? existing.Answerimage: null; // Keep old filename by default
            string? originalFileName = isFile? existing.Filename: null;

            if (file != null && file.Length > 0)
            {
                // Delete old file if it exists
                if (!string.IsNullOrEmpty(existing.Answerimage))
                {
                    var oldFilePath = Path.Combine(
                        Directory.GetCurrentDirectory(),
                        "wwwroot", "uploads", "assignments",
                        existing.Answerimage);

                    if (System.IO.File.Exists(oldFilePath))
                    {
                        System.IO.File.Delete(oldFilePath);
                        Console.WriteLine($"[UPDATE ASSIGNMENT] Deleted old file: {existing.Answerimage}");
                    }
                }
            ...
            existing.Description = description ?? existing.Description;
            existing.Date = DateTime.Now.ToShamsi();
            existing.Filename = originalFileName;  // ✓ UPDATE ORIGINAL FILENAME
            existing.Answerimage = fileName;       // ✓ UPDATE GENERATED FILENAME
        ...
        // Submit exam answer
        // ──────────────────────────────────────────────────────────────
        [HttpPost("submit/exam/{userId}/{examId}/{isFile}")]
        public async Task<IActionResult> SubmitExam(
            long userId,
            long examId,
            bool isFile,
            ...
            [FromForm] string? isUpdate = "false")
        {
            try
            {
                ...
                // Handle file upload
                string? fileName = isFile? existing?.Answerimage: null; // Keep old filename by default for updates
                string? originalFileName = isFile? existing?.Filename: null;
                ...
                if (file != null && file.Length > 0)
                {
                    ...
                    // Delete old file if updating
                    if (isUpdating && !string.IsNullOrEmpty(existing?.Answerimage))
                    {
                        try
                        {
                            var oldFilePath = Path.Combine(uploadsFolder, existing.Answerimage);
                            if (System.IO.File.Exists(oldFilePath))
                                System.IO.File.Delete(oldFilePath);

                            Console.WriteLine($"[SUBMIT EXAM] Deleted old file: {existing.Answerimage}");
                        }
                        catch (Exception ex)
                        {
                            Console.WriteLine($"[SUBMIT EXAM] Error deleting old file: {ex.Message}");
                ...
                if (isUpdating && existing != null)
                {
                    ...
                    existing.Date = DateTime.Now.ToShamsi();
                    existing.Time = DateTime.Now.ToString("HH:mm:ss");
                    existing.Filename = originalFileName;  // ✓ SAVE ORIGINAL FILENAME
                    existing.Answerimage = fileName;       // ✓ SAVE GENERATED FILENAME
